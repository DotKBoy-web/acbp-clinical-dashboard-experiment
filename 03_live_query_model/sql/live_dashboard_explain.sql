/* ============================================================================
 LIVE DASHBOARD KPI QUERY – EXPLAIN ANALYZE (FAC_01 – IPD)
 Baseline "Live SQL" performance measurement
 ============================================================================ */
...
enc_flags AS (
  SELECT
    es.*,
    CASE WHEN es.inpatient_admit_dt_tm >= b.start_ts AND es.inpatient_admit_dt_tm < b.end_ts THEN 1 ELSE 0 END AS f_admit_today,
    CASE WHEN es.disch_dt_tm >= b.start_ts AND es.disch_dt_tm < b.end_ts THEN 1 ELSE 0 END AS f_disch_today,
    CASE WHEN es.inpatient_admit_dt_tm <= now() AND (es.disch_dt_tm IS NULL OR es.disch_dt_tm > now()) THEN 1 ELSE 0 END AS f_census_live,
    CASE WHEN es.inpatient_admit_dt_tm <= now()
           AND (es.disch_dt_tm IS NULL OR es.disch_dt_tm > now())
           AND es.loc_room_cd IS NOT NULL
           AND es.loc_bed_cd IS NOT NULL
         THEN 1 ELSE 0 END AS f_bedded_census_live
  FROM enc_scope es
  CROSS JOIN bounds b
),
-- Unit-level occupied beds (use tuple distinct to avoid string concat artifacts)
unit_census_live AS (
  SELECT
...
capacity_kpis AS (
  SELECT
    ...
    ROUND(
      CASE WHEN SUM(uc.cap_beds) = 0 THEN 0
  ...
  JOIN cerner_ref.nurse_unit nu ON nu.nurse_unit_cd = uc.nurse_unit_cd
  JOIN cerner_ref.building b ON b.building_cd = nu.building_cd
  JOIN cerner_ref.facility f ON f.facility_cd = b.facility_cd
  LEFT JOIN unit_census_live ucl ON ucl.loc_nurse_unit_cd = uc.nurse_unit_cd
...
overall_kpis AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN f_bedded_census_live = 1 THEN (loc_room_cd, loc_bed_cd) ELSE NULL END) AS bedded_census_live,
    ROUND(
      AVG(CASE WHEN disch_dt_tm IS NOT NULL THEN EXTRACT(EPOCH FROM (disch_dt_tm - inpatient_admit_dt_tm)) / 86400.0 END),
      2
    ) AS avg_los_days
  FROM enc_flags
),
...
discharge_order_times AS (
  ...
  JOIN cerner.order_catalog oc
    ON oc.catalog_cd = o.catalog_cd AND oc.catalog_type_cd = o.catalog_type_cd
...
discharge_time_kpis AS (
  ...
  FROM enc_flags ef
  JOIN discharge_order_times dot ON dot.encntr_id = ef.encntr_id
