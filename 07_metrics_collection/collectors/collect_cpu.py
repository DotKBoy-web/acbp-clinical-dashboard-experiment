...
"""
collect_cpu.py
Samples Docker container CPU usage (%).

Outputs:
  ACBP_Clinical_Dashboard_Experiment/07_metrics_collection/raw_logs/cpu.csv
"""
...
def main():
    ...
    out = root / "07_metrics_collection" / "raw_logs" / "cpu.csv"
    header = ["timestamp_utc", "cpu_percent"]

    for i in range(args.samples):
        ts = utc_now_iso()
        try:
            cpu = docker_cpu_percent(args.container)
            append_row(out, header, [ts, cpu])
        except Exception:
            append_row(out, header, [ts, "ERROR"])
